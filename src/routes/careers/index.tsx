...
import heroImg from "@/assets/bg-careers-building.jpg";
            ...
            <div className="sticky top-24 rounded-2xl border border-border bg-card p-5 shadow-xs">
              <div className="flex items-center justify-between border-b border-border pb-4 mb-4">
                <span className="flex items-center gap-2 text-sm font-bold text-foreground">
                  <Filter className="h-4 w-4 text-[#1e7f7c] dark:text-[#38bdf8]" />
                  <span>Filter Roles</span>
                </span>
                {isAnyFilterActive && (
                  <button
                    onClick={clearAllFilters}
                    className="text-xs font-semibold text-[#1e7f7c] dark:text-[#38bdf8] hover:underline cursor-pointer"
                  >
                    Clear All
                  </button>
                )}
              ...
              <div className="space-y-4">
                ...
                <div className="border-b border-border pb-4">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`h-4 w-4 text-[#1e7f7c] dark:text-[#38bdf8] transition-transform duration-200 ${
                      ...
                      {[
                        ...
                        >
                          <input
                            ...
                            className="text-[#1e7f7c] accent-[#1e7f7c] h-3.5 w-3.5"
                          />
                      ...
                      {industryOptions.map((ind) => (
                        ...
                        >
                          <input
                            ...
                            className="rounded text-[#1e7f7c] accent-[#1e7f7c] h-3.5 w-3.5"
                          />
                          <span>{ind}</span>
                        </label>
                      ))}
          ...
          <main className="lg:col-span-9 space-y-5">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-card border border-border p-3 sm:p-4 rounded-2xl shadow-xs">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-xl bg-background border border-border pl-10 pr-4 py-2.5 text-sm text-foreground placeholder-muted-foreground outline-none focus:border-[#1e7f7c] dark:focus:border-[#38bdf8] focus:ring-2 focus:ring-[#1e7f7c]/20"
                />
              </div>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-3 px-1">
                ...
                {isAnyFilterActive && (
                  <button
                    onClick={clearAllFilters}
                    className="inline-flex items-center gap-1 text-xs font-semibold text-[#1e7f7c] dark:text-[#38bdf8] hover:underline cursor-pointer"
                  >
            ...
            {filteredJobs.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-border bg-card p-12 text-center">
                ...
                <button
                  onClick={clearAllFilters}
                  className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#1e7f7c] dark:bg-[#38bdf8] dark:text-black px-4 py-2 text-xs font-semibold text-white transition-all cursor-pointer"
                >
                  Clear All Filters
                </button>
              </div>
              ...
              <div className="space-y-4">
                {filteredJobs.map((job) => {
                  ...
                  return (
                    <article
                      key={job.id}
                      className="group relative rounded-2xl border border-border bg-card p-6 shadow-xs transition-all duration-200 hover:border-[#1e7f7c]/50 dark:hover:border-[#38bdf8]/50 hover:shadow-md"
                    >
                      {/* Top Row: Title, Metadata Badges & Actions */}
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                        <div>
                          <div className="flex flex-wrap items-center gap-2 mb-2">
                            <span className="rounded-md bg-[#1e7f7c]/10 dark:bg-[#38bdf8]/15 px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider text-[#1e7f7c] dark:text-[#38bdf8]">
                              {job.department}
                            </span>
                          ...
                          <Link
                            to="/careers/$jobId"
                            params={{ jobId: job.id }}
                            className="text-lg sm:text-xl font-bold text-foreground group-hover:text-[#1e7f7c] dark:group-hover:text-[#38bdf8] transition-colors"
                          >
                            {job.title}
                          </Link>
                        </div>
                        ...
                        <div className="flex items-center gap-2 self-start sm:self-auto shrink-0">
                          ...
                          >
                            {isSaved ? (
                              <BookmarkCheck className="h-4 w-4 text-[#1e7f7c] dark:text-[#38bdf8]" />
                            ) : (
                          ...
                          <Link
                            to="/careers/$jobId"
                            params={{ jobId: job.id }}
                            className="inline-flex items-center gap-1.5 rounded-xl bg-[#1e7f7c] dark:bg-[#38bdf8] dark:text-black hover:bg-[#176a67] dark:hover:bg-[#0ea5e9] text-white px-4 py-2 text-xs sm:text-sm font-semibold shadow-xs transition-all cursor-pointer group/btn"
                          >
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1.5">
                          <Building2 className="h-3.5 w-3.5 text-[#1e7f7c] dark:text-[#38bdf8]" />
                          <span className="font-semibold text-foreground">HNR Global PVT LTD</span>
                        </span>

                        <span className="flex items-center gap-1.5">
                          <MapPin className="h-3.5 w-3.5 text-[#1e7f7c] dark:text-[#38bdf8]" />
                          <span>{job.location}</span>
                        </span>

                        <span className="flex items-center gap-1.5">
                          <Clock className="h-3.5 w-3.5 text-[#1e7f7c] dark:text-[#38bdf8]" />
                          <span>{job.experience}</span>
                        </span>

                        <span className="flex items-center gap-1.5 text-foreground font-semibold">
                          <Coins className="h-3.5 w-3.5 text-[#1e7f7c] dark:text-[#38bdf8]" />
